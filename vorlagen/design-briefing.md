...
selbst ist NICHT teilbar (braucht deinen Login). Brauchst du einen
schickbaren Link: in Claude Design „Export → Project archive" (gratis,
sofort, alle Seiten) und `npm run demo -- --datei "<Archiv.zip>" --kunde
"<Name>"` – fertig verpackt (Kanbuk-Leiste, unsichtbar für Google,
Handy-Hinweis) für Vercel. Kein Standalone-Export nötig (der kostet
Claude-Kontingent).
